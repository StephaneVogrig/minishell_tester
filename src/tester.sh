...
if [ ! -x $MINISHELL_PATH ]; then
	clear
	printf "\e[31m$MINISHELL_PATH does not exist.\e[0m\n\n"
	printf "\e[32mEnter the new path to minishell.\e[0m\n"
	echo
	read path
	replace "$MINISHELL_PATH" "$path" -- ./tester
	MINISHELL_PATH=$path
fi
